...
def update_post(post_type, post_id):
    ...
    if len(post_title) == 0:
        return jsonify({"success": False, "message": "Post Title must not be empty"}), 400
    if validators.length(post_title, min=3) != True:
        return jsonify({"success": False, "message": "Post title must be three characters long"}), 400
    ...
    if len(post_content) == 0:
        return jsonify({"success": False, "message": "Post content must not be empty"}), 400
    ...
    try:
        db.update_item(
            ...
            UpdateExpression="SET post_title = :post_title, post_author = :post_author, post_author_link = :post_author_link, post_content = :post_content",
    ...
    if len(post_secret) == 0:
        ...
        if validators.length(post_secret, min=6) != True:
            return jsonify({"success": True, "message": "Post secret must be six characters long"}), 400
